...
function Sidebar() {
    ...
    const {state, addTask, removeTask,scheduleTask}  = useTasks();
    ...
    return <>
     {IsAddModalOpen && createPortal(<AddTaskModal  setIsAddModalOpen={setIsAddModalOpen}/>, document.body)}
     {IsScheduleModalOpen && createPortal(<ScheduleTaskModal taskToSchedule={selectedTask} setIsScheduleModalOpen={setIsScheduleModalOpen}/> , document.body)}
            <div className=" w-full h-full rounded-md shadow-xl px-4 py-4 dark:bg-slate-900 ">
                ...
                <div className="flex flex-col gap-2">
                    {state.bucket.map((task,i)=> <div className="p-1 relative text-center rounded-md border border-gray-900 dark:bg-slate-800 dark:text-" key={i}>{task.title} 
                        <button onClick={()=> handleSchedule(task)} className="absolute right-8 top-0.5 translate-y-0.5 cursor-pointer hover:bg-gray-100 rounded-sm p-1 dark:hover:text-gray-900 dark:hover:bg-violet-300"><RiCalendarScheduleLine/></button>
